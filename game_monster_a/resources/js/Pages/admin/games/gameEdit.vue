...
const titles = reactive({
  title_ru: props.game.name.title_ru,
  title_en: props.game.name.title_en
});
...
let categories = ref(props.game.categories);
...
const sendData = () => {
  ...
  router.post(`/admin/games/update/${props.game.id}`, data, {
    forceFormData: true,
    onSuccess : () => {
...
<template>
<AdminLayout>
  <TitlePage>Редактировать игру</TitlePage>
  ...
  <div class="w-full flex gap-4">
    <div class="flex-1">
      <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="mainImg">Основная
        картинка <span class="text-orange-500">(Чтобы изменить загрузите новую картинку)</span></label>
      <input ref="img"
             class="block w-full mb-5 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
             id="mainImg" type="file">
      <img class="h-[300px] object-cover w-full rounded-2xl" :src="game.image" alt="">
    </div>
    <div class="flex-1">
      <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="longImg">Широкая картинка для
        шапки <span class="text-orange-500">(Чтобы изменить загрузите новую картинку)</span></label>
      <input ref="imgLong"
             class="block w-full mb-5 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
             id="longImg" type="file">
      <img class="h-[300px] object-cover w-full rounded-2xl" :src="game.longImage" alt="">
    </div>
  </div>
  <p class="w-full text-left my-2">Название игры:</p>
