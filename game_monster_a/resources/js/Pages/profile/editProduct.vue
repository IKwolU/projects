...
const props = defineProps(['games','product']);
const page = usePage();

let timeRange = ref(0);
let type = ref(props.product.type);
let time = ref(0);
...
let game_id = ref(props.product.game_id ? props.product.game_id : null);
let name = ref(props.product.name);
let description = ref(props.product.description);
let price = ref(props.product.price);
let count = ref(props.product.count);
let category = ref(props.product.category);
let podCategory = ref(props.product.podCategory);
...
const sendData = () => {
  ...
  router.post(`/profile/productUpdate/${props.product.id}`, data, {
    onSuccess : () => {
  ...
  <main class="w-full h-full flex justify-center items-center">
    <div class="w-[514px] flex flex-col sm:px-4">
      ...
      <div class="mt-[28px] flex flex-col gap-[20px]">
        ...
        <select v-model="podCategory" v-if="category !== null" class="bg-[#2C2C2C] cursor-pointer text-white rounded-[6px] w-full">
        ...
        <input v-model="name" class="rounded-[6px] w-full placeholder-[#A5A5A5] bg-[#2C2C2C]" placeholder="Название товара" type="text">
        <p v-if="$page.props.errors.name" class="text-red-500 text-[12px] mt-[-10px]">
          {{ $page.props.errors.name }}
        </p>
        <textarea v-model="description" class="rounded-[6px] w-full placeholder-[#A5A5A5] bg-[#2C2C2C] h-[164px]" placeholder="Описание"></textarea>
        <p v-if="$page.props.errors.description" class="text-red-500 text-[12px] mt-[-10px]">
          {{ $page.props.errors.description }}
        </p>
        <div v-if="type === 1" class="w-full relative">
          <input v-model="time" @change="getPolishing(time)" class="rounded-[6px] w-full pr-[37px] pt-[25px] placeholder-[#A5A5A5] bg-[#2C2C2C]" type="text">
          <input v-model="timeRange" @change="getPolishing(timeRange)" step="1"  class="absolute bottom-0 left-0 w-[514px] h-0 appearance-none cursor-pointer" type="range">
        ...
        <div class="w-full relative">
          <input @change="checkPrice" v-model="price" class="rounded-[6px] w-full pr-[37px] placeholder-[#A5A5A5] bg-[#2C2C2C]" placeholder="Стоимость" type="number">
        ...
        <button @click="sendData()" class="bg-c_orange rounded-[6px] py-[18px] hover:opacity-70 transition-all">Обновить</button>
      </div>
    </div>
  </main>
</Layout>
