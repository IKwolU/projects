...
export class Client {
     ...
     * @return Успешно
     */
    createApplicationCommentManager(body: Body46 | undefined): Promise<Anonymous200> {
    ...
    protected processCreateApplicationCommentManager(response: Response): Promise<Anonymous200> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
     ...
     * @return Успешно
     */
    getParkApplicationsLogItemsManager(body: Body47 | undefined): Promise<Anonymous203> {
    ...
    protected processGetParkApplicationsLogItemsManager(response: Response): Promise<Anonymous203> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
    ...
    protected processGetNotificationsManager(response: Response): Promise<Anonymous206> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class Body19 implements IBody19 {
    ...
    toJSON(data?: any) {
        ...
        if (Array.isArray(this.metros)) {
            data["metros"] = [];
            for (let item of this.metros)
                data["metros"].push(item);
        }
        data["commission"] = this.commission;
        data["fuel_type"] = this.fuel_type;
        data["transmission_type"] = this.transmission_type;
    ...
    init(_data?: any) {
        if (_data) {
            ...
            if (Array.isArray(_data["bookings"])) {
                this.bookings = [] as any;
                for (let item of _data["bookings"])
                    this.bookings!.push(Bookings3.fromJS(item));
            }
            this.self_employed_discount = _data["self_employed_discount"];
            if (Array.isArray(_data["cars"])) {
                this.cars = [] as any;
...
export class Bookings3 implements IBookings3 {
    /** идентификатор брони */
    id?: string;
    /** Подтверждение найма */
    hire_confirmed?: string;

    [key: string]: any;

    constructor(data?: IBookings3) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): Bookings3 {
        data = typeof data === 'object' ? data : {};
        let result = new Bookings3();
        result.init(data);
        return result;
    }
